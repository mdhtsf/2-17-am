...
export async function verifyRoutes(root, container, assert, npcId = 'kai') {
  const check = (condition, label) => assert(condition, `${npcId}: ${label}`)
  const start = npcId === 'kai' ? 'counter' : 'notes_spot'
  const interruptTarget = npcId === 'kai' ? 'shelf' : 'drink_area'
  const pointId = location => getNpcWaypoint(npcId, location)
  const walkSelector = `.${npcId}-walk`
  ...
  const render = destination => act(async () => root.render(<StrictMode>
    <div style={{position:'relative', width:1000, height:1000 / 1.5}}>
      <NPC npc={{id:npcId,name:npcId.toUpperCase()}} visual={npcVisuals[npcId]} anchor={sceneWaypoints[pointId(destination)]}
  ...
  try {
    ...
    const shadow = entity.querySelector('.npc-ground-shadow')
    check(Boolean(shadow) && getComputedStyle(shadow).pointerEvents === 'none', 'human has a non-intercepting ground shadow inside entity')
    await render('window')
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      check(movement.phase === 'idle' && timers.size === 0, 'reduced motion goes directly to final anchor without route timers')
    } else {
      const expected = resolveNpcRoute(pointId(start),pointId('window'))
      const walk = entity.querySelector(walkSelector)
      const firstFrame = walk.querySelector('.walking-frame')
      const frameAnimation = firstFrame.getAnimations()[0]
      let previous = sceneWaypoints[pointId(start)]
      for (const next of expected) {
        ...
        check(entity.querySelector(walkSelector) === walk && firstFrame.getAnimations()[0] === frameAnimation, `${next.id}: step animation continues without remount or restart`)
      ...
      check(clicks === expected.length && movement.phase === 'settling', 'human stays clickable throughout route and settles only at final arrival')
      await act(async () => { await new Promise(resolve => originalSet(resolve, KAI_MOVEMENT.settleMs + 30)) })
      check(movement.phase === 'idle' && timers.size === 0 && !entity.querySelector(walkSelector), 'full route completion leaves idle with no timers')
      await render(start)
      const obsolete = [...timers.values()][0]
      const before = getComputedStyle(entity).left
      await render(interruptTarget)
      await act(async () => obsolete())
      check(movement.destination === interruptTarget && movement.phase === 'walking' && timers.size === 1, 'latest destination replaces old route and rejects its stale completion')
      ...
      check(movement.route.at(-1) === pointId(interruptTarget) && !movement.route.includes(pointId(start)), 'interruption discards abandoned destination nodes')
    }
