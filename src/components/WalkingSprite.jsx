...
export default function WalkingSprite({ visual, movement, getWalkingVisual, npcId, registration }) {
  const { direction, phase, segmentTo, segmentFrom, destination } = movement
  const walk = getWalkingVisual(direction)
  return <span className={`walking-visual ${npcId}-visual`} data-direction={direction || 'idle'} data-phase={phase} data-waypoint={segmentTo} data-segment-from={segmentFrom} data-destination={destination}
